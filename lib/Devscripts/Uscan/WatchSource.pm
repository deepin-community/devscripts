...
foreach (qw(force_repack type upstream_url newfile_base releaseonly)) {
    has $_ => (is => 'rw');
}
...
sub parse {
    ...
    foreach my $optName (sort keys %$opts) {
        ...
        # releaseonly
        # EOF
        elsif ($optName =~ /^(?:re(?:leaseonly|pack)|decompress)$/) {
        ...
        # EOF
            ...
            =~ /^(?:(?:(?:search|pgp)?m|hrefdec)ode|c(?:omponent|type)|git(?:pretty|date)|repacksuffix|unzipopt)$/
        ) {
            $self->$optName($optVal);
        ...
        } elsif ($optName ne 'versionconstraint') {
    ...
    unless ($filepattern) {
        ...
        $filepattern
          = '.*?'
          . &Devscripts::Uscan::WatchSource::Transform::ANY_VERSION
          . &Devscripts::Uscan::WatchSource::Transform::ARCHIVE_EXT;
    }
...
=item L<Devscripts::Uscan::Modes::Http>::http_search()
=item L<Devscripts::Uscan::Modes::Ftp>::ftp_search()
=item L<Devscripts::Uscan::Modes::Git>::git_search()
=item L<Devscripts::Uscan::Modes::Svn>::svn_search()
...
=item L<Devscripts::Uscan::Modes:Http>::http_upstream_url()
=item L<Devscripts::Uscan::Modes:Ftp>::ftp_upstream_url()
=item L<Devscripts::Uscan::Modes:Git>::git_upstream_url()
=item L<Devscripts::Uscan::Modes:Svn>::svn_upstream_url()
...
=item L<Devscripts::Uscan::Modes:Http>::http_newfile_base()
=item L<Devscripts::Uscan::Modes:Ftp>::ftp_newfile_base()
=item L<Devscripts::Uscan::Modes:Git>::git_newfile_base()
=item L<Devscripts::Uscan::Modes:Svn>::svn_newfile_base()
...
=item L<Devscripts::Uscan::Modes:Http>::http_clean()
=item L<Devscripts::Uscan::Modes:Ftp>::ftp_clean()
=item L<Devscripts::Uscan::Modes:Git>::git_clean()
=item L<Devscripts::Uscan::Modes:Svn>::svn_clean()
...
sub _do {
    ...
    $mode =~ s/git-dumb/git/;
    $sub = $mode . "_$sub";
    with('Devscripts::Uscan::Modes::' . ucfirst(lc $mode))
      unless ($self->can($sub));
    if ($@) {
