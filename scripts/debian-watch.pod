...
Using B<Pretty: describe> also sets B<Git-Mode: full> to make a full local
clone of the repository automatically.

=item B<Date:> I<< <rule> >>

Set the date string used by the B<pretty> option to an arbitrary format when
...
adding B<Pretty> and B<Date> options.
